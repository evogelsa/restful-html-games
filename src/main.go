...
func homePage(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(
		w,
		"<div><a href=\""+ROOT_PATH+"/ttt\">click to play tictactoe!</a></div>",
	)
	fmt.Fprintf(
		w,
		"<div><a href=\""+ROOT_PATH+"/checkers\">click to play terrible checkers</a></div>",
	)
}
...
func newRouter() *mux.Router {
	...
	router.HandleFunc(ROOT_PATH+"/tttTurn/{row}/{col}", tictactoe.Turn).
		Methods("GET")
	...
	router.HandleFunc(ROOT_PATH+"/checkersTurn/{row}/{col}", checkers.Turn).
		Methods("GET")
	...
	router.HandleFunc(ROOT_PATH+"/checkersPlayer", checkers.PlayerSwitch).
		Methods("GET")
