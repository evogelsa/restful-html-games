	...
	for i := 0; i < len(board); i++ {
		for j := 0; j < len(board[0]); j++ {
			...
			if replaced[0] != "BLANK" {
				tokens := []interface{}{
					fmt.Sprintf(
						`<a href="/checkersTurn/%s/%s" style="text-decoration:none">%s</a>`,
						strconv.Itoa(i),
						strconv.Itoa(j),
						replaced[0],
					),
...
func Turn(w http.ResponseWriter, r *http.Request) {
	...
	mutex.Unlock()
	http.Redirect(w, r, "/checkers", http.StatusFound)
}
...
func Board(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(
		...
		<div><h3>%s</h2></div>
		<div><h4><a href="/checkersPlayer">(switch player / double jump)</a></h4></div>
		...
		<div><h4><a href="/checkersUndo">(undo last move)</a></h4></div>
		<div><h4><a href="/checkersReset">(reset game)</a></h4></div>
